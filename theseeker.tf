...
resource "aws_instance" "theseeker" {
  ...
  /* provisioners */
    ...
    provisioner "remote-exec" {
      ...
      /* here we add the default cookbook and the default hostsfile recipe containing the chef client address */
      "knife node --local-mode run_list add localhost 'recipe[ntp::default], recipe[hostsfile::default]'"
      ]
      connection {
        type = "ssh"
        user = "ec2-user"
